...
output:
  html_document
params:
  quantif_file: "D:/Utilisateurs/cludwig/stage/RStudio/Tests/Data/Quanti/Parsed_data/Human_yeast/all_mrf_withsamplename.txt"
  design_exp: "../../Data/Quanti/Parsed_data/Human_yeast/designExp.txt"
  parameters: "../../Data/Quanti/Parsed_data/Human_yeast/Params_da.txt"
  output_table: "..."
...
if(parameters["Test.type",]%in%c("anova","kruskall")){
  ...
  reshape_imputed_abundance = melt(intensity_df,id="Id")
  reshape_imputed_abundance$variable = sub(".*_","",sub(paste0(pattern,".*"),"",reshape_imputed_abundance$variable))
  
  for(id in imputed_abundance$Id){
    
    current_protein_intensities = subset(reshape_imputed_abundance,reshape_imputed_abundance$Id==id)
    ...
    if(parameters["Test.type",]=="anova"){
      ...
      if(parameters["Test.type",]=="kruskall"){
        # KRUSKAL
        kruskal = kruskal.test(reshape_imputed_abundance$value~as.factor(reshape_imputed_abundance$variable))
        pval = kruskal$p.value
        # Wilcoxon pairwise
        res = pairwise.wilcox.test(reshape_imputed_abundance$value,reshape_imputed_abundance$variable,p.adjust.method="none")
      }
    }
...
for (comparison in comparisons_to_do){
  ...
  if(as.logical(parameters["Test.paired",])){
    ...
    totIntensities[paste0("mean_intensity_",A_name)] = rowMeans(log2(data.matrix(totIntensities[grepl(A_name,names(totIntensities))])),na.rm = TRUE)
    totIntensities[paste0("mean_intensity_",B_name)] = rowMeans(log2(data.matrix(totIntensities[grepl(B_name,names(totIntensities))])),na.rm = TRUE)
    totIntensities[paste0("fc_",comparison)] = 2^(totIntensities[paste0("mean_intensity_",A_name)]-totIntensities[paste0("mean_intensity_",B_name)])
  }
  ...
  if(comparison%in%comparisons_to_display){
    ...
    MA = ggplot(current_statDF, aes(x=log10(mean_intensity), y=log2(abs(fc)))) +
      geom_point() +
      ggtitle("MA plot")+
    ...
    if(value=="qval" & Fold.Change=="fc"){
      Volcano = ggplot(current_statDF, aes(x=log2(fc), y=-log10(as.numeric(qval))))
    }else if(value=="pval" & Fold.Change=="fc"){
      Volcano = ggplot(current_statDF, aes(x=log2(fc), y=-log10(as.numeric(current_statDF$pval))))
    }else if(value=="qval" & Fold.Change=="zscore"){
      Volcano = ggplot(current_statDF, aes(x=zscore, y=-log10(as.numeric(qval))))
    }else{
      Volcano = ggplot(current_statDF, aes(x=zscore, y=-log10(as.numeric(pval))))
    }
  
    Volcano = Volcano +
      geom_point(aes(colour=current_statDF$significant),size=3) +
      ...
      geom_text(aes(label=ifelse(current_statDF$significant==T,as.character(current_statDF$Label),'')),hjust=1,vjust=-1, size=6)+
      ggtitle("")+
  ...
  #assign(paste0("log10_pval_",comparison),-log10(current_statDF[paste0("pval_",comparison)]))
  #log10_pval = -log10(current_statDF[paste0("pval_",comparison)])
  ...
  #assign(paste0("log10_qval_",comparison),-log10(current_statDF[paste0("qval_",comparison)]))
  ...
  # log = -log10(current_statDF[paste0("qval_",comparison)])
  # current_statDF = add_column(current_statDF, log, .after = paste0("qval_",comparison))

  #assign(paste0("log2_fc_",comparison),log2(current_statDF[paste0("fc_",comparison)]))
  ...
  # log2 = log2(current_statDF[paste0("fc_",comparison)])
  # current_statDF = add_column(current_statDF, log2, .after = paste0("fc_",comparison))
...
data.table::fwrite(final_data, output_table, row.names = F, sep = "\t")
