...
params:
  quant_df: "../../../QE240723_TEvsCt_240916_2024-09-17_1706/QE240723_TEvsCt_240916_2024-09-17_1706.tsv"
  parameters: "../../../QE240723_TEvsCt_240916_2024-09-17_1706/stats_parameters.txt"
  design_exp: "../../../QE240723_TEvsCt_240916_2024-09-17_1706/QE240723_TEvsCt_240916_2024-09-17_1706_exp_design.txt"
  output_dir: "../../../QE240723_TEvsCt_240916_2024-09-17_1706/my_modification"
...
#library(tidyr)
#data <- data %>% select(!Gene_name) %>% separate(Accession,into = c("Gene_name","tmp"),sep = "\\.",remove = F) %>% select(!tmp)
...
Vol_labels <- strsplit(as.character(parameters["Volcano.manual.accession",]),";")[[1]]
...
for(group in group_ids){
  ...
  for (rBio in unique(sub$repBio)){
    ...
    if(length(samples_intensities)>1){
      ...
      #id_type2 = data %>% mutate(samples_identification_type=ifelse("By MS/MS"%in%samples_identification_type ,"By MS/MS","By matching"))
...
#filtered_raw_abundance <-raw_abundance
...
#imputed_abundance <- global_imputed_abundance[global_imputed_abundance$Id %in% data$Id, ]
global_imputed_abundance_log <- imputed_abundance_log
imputed_abundance_log <- global_imputed_abundance_log[global_imputed_abundance_log$Id %in% data$Id, ]
...
df_median_deviation_before_processing = seq(1,nrow(filtered_data)) #data
df_median_deviation_after_processing = seq(1,nrow(filtered_data)) #data

df_cv_before_processing = seq(1,nrow(filtered_data)) #data
df_cv_after_processing = seq(1,nrow(filtered_data)) #data
...
for (group in group_ids){
  ...
  # Before processing
  intensities = filtered_data[grep(paste0(group,"_mean$"),names(filtered_data),value=T)] #data
  ...
  # After processing
  intensities = filtered_data[grepl(paste0(group,".*_imputed$"),names(filtered_data))] #data
...
########################
#comparisons_to_display <- "C2_CRK/C3_LRR"
...
statDF = data.frame(filtered_data$Id,filtered_data$Accession,filtered_data$Label) #data
colnames(statDF) = c("Id","Accession","Label")
 ## STAT_DF before using mapping with previous comparisons
precv_statDF <- statDF
...
for (comparison in comparisons_to_do){
  ...
  for(group in couple){
    ...
    sub = subset(expDesign,expDesign$Condition==group)
    sel_global_mean_abundance_repbio <- global_mean_abundance_repbio %>% select(Id,contains(group))
    
    filtering_data <- filtering_data %>% left_join(sel_global_mean_abundance_repbio,by="Id")
      #cbind(filtering_data, global_mean_abundance_repbio[grepl(group, colnames(global_mean_abundance_repbio))])
  ...
  statDF = merge.data.frame(precv_statDF,totIntensities[,c("Accession",paste0("mean_intensity_",comparison),paste0("fc_",comparison))],by="Accession")
  ...
  if(exists('pvalue_table')){
    ...
    if(parameters["Test.type",]=="limma"){
      ...
      ##### TO Get Rid of this error: "Error in `.rowNamesDF<-`(x, value = value) : \n duplicate 'row.names' are not allowed
      if(sum(isUnique(filtered_data$Accession)) == length(filtered_data$Accession)){
        rownames(data_limma)=filtered_data$Accession

      }else{
        unique_rownames <- filtered_data %>%
          mutate(rowname = ifelse(duplicated(Accession) | duplicated(Accession, fromLast = TRUE),
                            paste(Accession, Id, sep = "_"),
                            Accession)) %>%
          remove_rownames() %>%
          column_to_rownames(var = "rowname")
        
        row.names(data_limma) <- row.names(unique_rownames)
        }
      ...
      for(prot in 1:nrow(A)){
        ...
        statDF[paste0("pval_",comparison)][match(A[prot,1],statDF$Accession,nomatch =0),]<- pval
      }
  ...
  #current_statDF = statDF[grepl(paste0("Accession|Label|",comparison),colnames(statDF))]
   #assign(paste0("curr_statDF_",comparison),current_statDF)
  ...
  if(comparison%in%comparisons_to_display){
    ...
    top_50_pval = current_statDF %>% filter(significant==TRUE) %>% slice_min(pval, n = 50)
    ##Framing of MA plot using ggplot##
    ...
    MA = ggplot(current_statDF, aes(x=log10(mean_intensity), y=log2(fc))) +
      ...
      {if(!monochrome && !col_sig) scale_color_gradientn(colors = col_list, values = c(0, .5, 1))}+ #colors= rev(col_list)
      {if(col_sig) scale_color_manual(values = sig_list)}+
      {if(!lblng_mnl) ggrepel::geom_text_repel(data = top_50_pval,aes(label=as.character(top_50_pval$Label)),
      ...
      {if(!lblng_mnl) labs(subtitle = "Top 50 significant hits were labeled.")} +
      geom_hline(yintercept=Volcano.threshold.fc,linetype="dashed") +
      geom_hline(yintercept=-Volcano.threshold.fc,linetype="dashed") +
      # scale_colour_manual(values = c("black", "tomato1"))+
      ggtitle("MA plot")+
      xlab("log 10(Intensity)")+ # "log maximum intensity"
    ...
    if(length(current_statDF$significant[current_statDF$significant==T])>1){
        ...
        imputed = cbind.data.frame(raw_intensities_significant_proteins[,1:2],is.na(raw_intensities_significant_proteins [,-c(1,2)])) # Second column was selected 
                                                                                                                                       #additionally to remove duplicates in the
                                                                                                                                       #heatmap function
...
if(params$proline_source_file == "none"){
  data.table::fwrite(output_t,paste0(params$output_dir,"output_table.tsv"), row.names = F, sep = "\t")
  ...
  exit_address <- paste0(as.character(params$output_dir), "aft_rmv_global_filt_DA_output_table.xlsx")
